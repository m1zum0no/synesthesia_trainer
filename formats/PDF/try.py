...
# Page.write_text() which combines one or more TextWriters and jointly writes them to a given rectangle 
# and with a given rotation angle (morph parameter of TextWriter.write_text()).
        ...
        if not block['type']:  # 0 for txt
            for line in block['lines']:
                for span in line["spans"]:
                    ...
                    text_span = fitz.Rect(span["bbox"])
                    the_fontsize = scale_fontsize(span, text_span, the_font)
                    first_letter = span['chars'][0]
                    head_tw = fitz.TextWriter(text_span, color=color_letter(first_letter['c']))
                    head_tw.append(first_letter['origin'], first_letter['c'], font=the_font, fontsize=the_fontsize)
                    prev_tw = head_tw 
                    for ch in span['chars'][1:]:
                        ...
                        curr_tw = fitz.TextWriter(text_span, color=color_letter(ch['c']))
                        curr_tw.append(prev_tw.last_point, ch['c'], font=the_font, fontsize=the_fontsize)
                        curr_tw.write_text(page)
                        prev_tw = curr_tw
