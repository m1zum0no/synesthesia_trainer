...
with open('mod-opencv_chars_pos.txt', 'a+') as output:
  with open('chars_pos.txt', 'r') as input:
      ...
      for word_data in re.finditer(r'word (?P<coords>[0-9 ]+)', line):
        ...
        if next_word := re.search(r'word (?P<coords>[0-9 ]+)', line[word_start:]):
          ...
          for char_bbox in word_bbox.chars:
            ...
            color = color_table.get(char_bbox.char.lower(), '#000000').lstrip('#')
            color = ' '.join([str(int(color[i:i+2], 16)) for i in (0, 2, 4)])
            char_position_and_color = ' '.join(map(str, (char_bbox.x0, char_bbox.y0, char_bbox.x0 + w, char_bbox.y0 + h, color)))
            output.write(char_position_and_color + '\n')
